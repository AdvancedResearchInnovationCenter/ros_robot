...
class AbbRobot(Robot):
    ...
    def move_TCP(self, pose_vec, vel, acc, slow=False):
        ...
        abb_R_ur = R.from_euler('z', 90, degrees=True).as_dcm()
        ...
        quat = R.from_dcm(abb_R_tcp).as_quat().tolist()
    ...
    def get_pose(self, force_reading=False):
        ...
        #Rotate poses by -90 deg to be compatible with base frame of UR10
        ur_R_abb = R.from_euler('z', -90, degrees=True).as_dcm()
        
        rot_mat = R.from_quat(quat[1:] + [quat[0]]).as_dcm()
